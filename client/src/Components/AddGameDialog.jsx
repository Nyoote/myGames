...
export default function AddGameDialog({ open, onClose, onSuccess }) {
    ...
    const validate = () => {
        ...
        if (!formData.plateforme.trim()) newErrors.plateforme = "At least one platform is required";
        if (!formData.editeur.trim()) newErrors.editeur = "Publisher is required";
        if (!formData.developpeur.trim()) newErrors.developpeur = "Developer is required";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!data) {
                ...
                const msg = data.error || data.message || "Error, cannot add game";
                setServerError(msg);
            }
        } finally {
            setSubmitting(false);
